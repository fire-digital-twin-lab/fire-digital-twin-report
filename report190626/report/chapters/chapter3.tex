...
\begin{tikzpicture}[
  ...
  \node[fdsblock] (fds) at (3.2, 1.2) {FDS Reference Cases};
  
  \node[block] (post) at (6.4, 2.4) {Post-processing};
  \node[fdsblock] (compare) at (6.4, 1.2) {FDS Post-processing\\Aligned Sensors + Labels};
  \node[fdsblock] (fdsref) at (9.6, 0.7) {FDS Reference Set};
  ...
  % --- Đường nối Hàng 3 ---
  \draw[arrow] (ai) -- (eval);
  \draw[redarrow, dashed] (fdsref.south) |- (eval.north);
  \draw[arrow] (eval) -- (api);
  \draw[arrow] (api) -- (dash);
